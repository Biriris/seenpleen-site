...
import 'piccolore';
import { c as createRenderInstruction, r as renderComponent, a as renderTemplate, m as maybeRenderHead, b as addAttribute } from './prerender_zTfVk3HF.mjs';
...
const $$Index = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "BaseLayout", $$BaseLayout, { "title": "SeenPleen Studio" }, { "default": ($$result2) => renderTemplate` ${renderComponent($$result2, "Header", $$Header, {})} ${featuredProject && featuredImage && renderTemplate`${maybeRenderHead()}<section class="home-hero"> <img${addAttribute(featuredImage, "src")}${addAttribute(featuredProject.title, "alt")} class="home-hero-image" loading="eager"> <div class="home-hero-overlay"> <a${addAttribute(`/projects/${featuredProject.slug}/`, "href")} class="home-hero-link"> ${featuredProject.title} </a> </div> </section>`}<section class="projects-section" id="projects"> <div class="projects-heading"> <div> <span>Selected Works</span> <h2>Projects</h2> </div> ${categories.length > 0 && renderTemplate`<div class="projects-filters" aria-label="Project filters"> <button type="button" class="projects-filter is-active" data-filter="all">
All
    ...
    return renderTemplate`<a class="project-card"${addAttribute(`/projects/${p.slug}/`, "href")}${addAttribute(p.category || "", "data-category")}> <div class="project-card-image"> ${p.cover ? renderTemplate`<img${addAttribute(p.cover, "src")}${addAttribute(p.title, "alt")}${addAttribute(index === 0 ? "eager" : "lazy", "loading")}>` : renderTemplate`<div class="project-card-placeholder"></div>`} <div class="project-card-hover"> <div class="project-card-hover-inner"> <h3>${p.title}</h3> ${hoverMeta.length > 0 && renderTemplate`<p>${hoverMeta.join(" / ")}</p>`} </div> </div> </div> </a>`;
